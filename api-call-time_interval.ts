...
while (true) {
  ...
  const tempUrl = `${API_URL}?apikey=${API_KEY}&f.datum.end=${endCall}&f.datum.start=${startCall}${ cursor ? `&cursor=${cursor}` : '' }`;
  // const tempUrl = `${API_URL}?apikey=${API_KEY}&f.id=1`;
  try {
    ...
    if (iteration == 0) {
      ...
      console.log(calls + " calls needed");
      console.log("===================================");
      console.log();
    }
